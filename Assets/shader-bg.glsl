...
vec3 drawSquare (in vec2 p, in vec2 square, in vec3 setting) {
	if(boxtest(p, square, setting.x) && !(boxtest(p, square, setting.x - setting.y))) 
        return palette(setting.z / 70.0 + u_time * 0.1);
...
void main (void) {
    ...
    float a = sin(u_time) * 0.6; //box angle
	mat2 rot = mat2(cos(a), -sin(a), sin(a), cos(a));
	pos = uv * aspect * rot; 
	center = vec2(0.5) * rot;
	
	for(int i = 0; i < 50; i++) {
		vec3 d = drawSquare(pos, center + vec2(sin(float(i) / 10.0 + u_time) / 4.0, 0.0), vec3(0.0 + sin(float(i) / 200.0), 0.01 , float(i)));
		if(length(d) != 0.0) color = mix(d, vec3(1.0), 0.3); //slight desaturate and boost
	}
